...
ESTADO atualizarAjuda(ENTRADA *entrada, bool trocaDeEstado, double dt);
...
int main(void)
{
    ...
    */
    ...
    while (estado != FIM)
    {
        ...
        {
        case AJUDA:
            estado = atualizarAjuda(&entrada, trocaDeEstado, dt);
            break;
        case MENU:
...
ESTADO atualizarAjuda(ENTRADA *entrada, bool trocaDeEstado, double dt)
{
    ...
    //Estado que retorna quando o usuario avança.
    if (entrada->confirma)
    {
        estado = MENU;
    }

    // Calcula as coordenadas do texto das instruções.
    int ALTURA = 11;
    int LARGURA = 33;
    ...
    centralizarString(stdscr, origemY, "INSTRUÇÕES");
    attroff(A_UNDERLINE | A_BOLD);

    mvaddstr(origemY + 4, origemX, "Use W e S para mover o arqueiro");
    mvaddstr(origemY + 5, origemX, "Use a barra de espaço para atirar");
    mvaddstr(origemY + 6, origemX, "Pressione P para pausar o jogo");

    centralizarString(stdscr, origemY + 10, "Aperte ENTER ou ESPAÇO para ir para o menu");

    //Adiciona a janela à fila de janelas a serem atualizadas
    //no fim do quadro.
    wnoutrefresh(stdscr);
